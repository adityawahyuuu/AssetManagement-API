...
{
    internal sealed class UpdateAssetValidator : AbstractValidator<UpdateAssetDto>
    {
        public UpdateAssetValidator()
        {
            RuleFor(x => x.Name)
                .MaximumLength(255).When(x => !string.IsNullOrEmpty(x.Name))
                .WithMessage("Asset name must not exceed 255 characters");
            ...
            RuleFor(x => x.LengthCm)
                .GreaterThan(0).When(x => x.LengthCm.HasValue)
                .WithMessage("Length must be greater than 0");

            RuleFor(x => x.WidthCm)
                .GreaterThan(0).When(x => x.WidthCm.HasValue)
